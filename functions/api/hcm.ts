...
export const onRequestPost: PagesFunction<Env> = async ({ env, request }) => {
  try {
    ...
    return json({ error: 'unknown_action', allowed: ['create_candidate','update_candidate','save_scorecard','acknowledge_handbook','complete_training','update_goal','create_offboarding','advance_offboarding'] }, { status: 400 });
  } catch (error) { return json({ error: 'database_unavailable', detail: error instanceof Error ? error.message : 'unknown' }, { status: 503 }); }
};
