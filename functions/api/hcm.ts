...
const views = new Set(['summary','people','candidates','handbook','training','reviews','offboarding']);
...
export const onRequestGet: PagesFunction<Env> = async ({ env, request }) => {
  ...
  try {
    ...
    const [people, openCandidates, dueTraining, pendingAck, reviews, offboarding] = await Promise.all([
    ...
    return json({ boardId, view, data: { people, openCandidates, dueTraining, pendingAck, reviews, offboarding } });
...
export const onRequestPost: PagesFunction<Env> = async ({ env, request }) => {
  ...
  try {
    ...
    return json({ error: 'unknown_action', allowed: ['create_candidate','acknowledge_handbook','complete_training','create_offboarding','advance_offboarding'] }, { status: 400 });
  } catch (error) { return json({ error: 'database_unavailable', detail: error instanceof Error ? error.message : 'unknown' }, { status: 503 }); }
};
