...
export const onRequestPost: PagesFunction<Env> = async ({ env, request }) => {
  try {
    ...
    if (action === 'classify_transaction') {
      ...
      const result = await db.prepare("UPDATE bank_transactions SET status='approved',match_entity_type=NULL,match_entity_id=NULL,match_confidence=NULL,manual_counter_account_id=?,classification_note=?,reviewed_by=?,reviewed_at=datetime('now') WHERE id=? AND board_id=? AND status IN ('imported','suggested','approved') AND posted_voucher_id IS NULL").bind(counterAccountId, classificationNote, actor, transactionId, boardId).run();
