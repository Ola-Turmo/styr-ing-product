  ...
  try {
    ...
    if (action === 'link_payment') {
      ...
      const source = entityType === 'sales_invoice'
        ? await db.prepare("SELECT id,invoice_number,total_minor,status FROM sales_invoices WHERE id=? AND board_id=?").bind(entityId, boardId).first<Record<string, unknown>>()
        : await db.prepare("SELECT id,invoice_number,amount_minor,status FROM supplier_invoices WHERE id=? AND board_id=?").bind(entityId, boardId).first<Record<string, unknown>>();
      ...
      const bankAmount = Math.abs(Number(transaction.amount_minor || 0)); const sourceAmount = Math.abs(Number(source.total_minor ?? source.amount_minor ?? 0));
      if (!sourceAmount || sourceAmount !== bankAmount) return json({ error: 'bank_source_amount_mismatch', bankAmountMinor: bankAmount, sourceAmountMinor: sourceAmount }, { status: 409 });
      ...
      try { await db.prepare('INSERT INTO payment_links (id,board_id,bank_transaction_id,entity_type,entity_id,amount_minor,status,linked_by) VALUES (?,?,?,?,?,?,?,?)').bind(linkId, boardId, transactionId, entityType, entityId, bankAmount, 'linked', actor).run(); }
      ...
      if (entityType === 'sales_invoice') await db.prepare("UPDATE sales_invoices SET status='paid',paid_at=datetime('now'),payment_reference=? WHERE id=? AND board_id=?").bind(`BANK:${reference}`, entityId, boardId).run();
      if (entityType === 'supplier_invoice') await db.prepare("UPDATE supplier_invoices SET status='paid',paid_at=datetime('now'),payment_reference=?,approved_by=COALESCE(approved_by,?),approved_at=COALESCE(approved_at,datetime('now')) WHERE id=? AND board_id=?").bind(`BANK:${reference}`, actor, entityId, boardId).run();
      ...
      return json({ ok: true, action, paymentLinkId: linkId, transactionId, entityType, entityId, status: 'linked', glPosting: 'pending_controlled_posting' }, { status: 201 });
    }
