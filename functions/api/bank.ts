...
export const onRequestPost: PagesFunction<Env> = async ({ env, request }) => {
  try {
    ...
    if (action === 'post_match') {
      ...
      const sourceAmount = Math.abs(Number(source.total_minor ?? source.amount_minor ?? 0)); const remainingAmount = Math.max(0, sourceAmount - Number(source.paid_minor || 0)); if (!sourceAmount || amountMinor > remainingAmount) return json({ error: 'bank_source_amount_mismatch', bankAmountMinor: amountMinor, remainingAmountMinor: remainingAmount }, { status: 409 });
