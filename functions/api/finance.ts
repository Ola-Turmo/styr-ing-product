...
export const onRequestPost: PagesFunction<Env> = async ({ env, request }) => {
  try {
    ...
    if (action === 'record_invoice_payment') {
      ...
      const invoice=await db.prepare("SELECT i.total_minor,i.paid_minor,i.status,COALESCE((SELECT SUM(c.total_minor) FROM sales_credit_notes c WHERE c.sales_invoice_id=i.id AND c.board_id=i.board_id AND c.status='posted'),0) credited_minor FROM sales_invoices i WHERE i.id=? AND i.board_id=?").bind(invoiceId,boardId).first<Record<string,unknown>>(); if(!invoice || !['approved','sent','overdue'].includes(String(invoice.status))) return json({error:'invoice_not_payable_or_found'},{status:409});
      const total=Number(invoice.total_minor||0), alreadyPaid=Number(invoice.paid_minor||0), credited=Number(invoice.credited_minor||0), remaining=Math.max(0,total-alreadyPaid-credited), paymentAmount=requestedAmount ?? remaining; if(paymentAmount<=0 || paymentAmount>remaining) return json({error:'payment_exceeds_remaining',remainingMinor:remaining,creditedMinor:credited},{status:409});
