...
async function boardData(env: Env, boardId: string, view: string) {
  ...
  if (view === 'receivables') return (await db.prepare("SELECT i.id,i.invoice_number,i.issue_date,i.due_date,i.description,i.total_minor,i.status,i.paid_at,i.payment_reference,i.paid_minor,COALESCE((SELECT SUM(c.total_minor) FROM sales_credit_notes c WHERE c.sales_invoice_id=i.id AND c.board_id=i.board_id AND c.status='posted'),0) credited_minor,MAX(0,i.total_minor-i.paid_minor-COALESCE((SELECT SUM(c.total_minor) FROM sales_credit_notes c WHERE c.sales_invoice_id=i.id AND c.board_id=i.board_id AND c.status='posted'),0)) outstanding_minor,pl.id payment_link_id,pl.bank_transaction_id,pl.status payment_link_status,a.company_name,CASE WHEN MAX(0,i.total_minor-i.paid_minor-COALESCE((SELECT SUM(c.total_minor) FROM sales_credit_notes c WHERE c.sales_invoice_id=i.id AND c.board_id=i.board_id AND c.status='posted'),0))=0 THEN 'paid' WHEN i.status IN ('draft','review') THEN 'needs_review' WHEN i.due_date IS NOT NULL AND i.due_date < date('now') THEN 'overdue' ELSE i.status END AS collection_status FROM sales_invoices i LEFT JOIN crm_accounts a ON a.id=i.account_id LEFT JOIN payment_links pl ON pl.entity_type='sales_invoice' AND pl.entity_id=i.id AND pl.board_id=i.board_id WHERE i.board_id=? AND i.status NOT IN ('cancelled') ORDER BY CASE WHEN MAX(0,i.total_minor-i.paid_minor-COALESCE((SELECT SUM(c.total_minor) FROM sales_credit_notes c WHERE c.sales_invoice_id=i.id AND c.board_id=i.board_id AND c.status='posted'),0))=0 THEN 3 WHEN i.due_date < date('now') THEN 0 ELSE 1 END,i.due_date").bind(boardId).all()).results;
