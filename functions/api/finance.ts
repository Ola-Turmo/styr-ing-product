...
async function boardData(
  ...
  if (view === "receivables")
    return (
      await db
        .prepare(
          "SELECT i.id,i.invoice_number,i.issue_date,i.due_date,i.description,i.total_minor,i.status,i.paid_at,i.payment_reference,i.paid_minor,COALESCE((SELECT SUM(c.total_minor) FROM sales_credit_notes c WHERE c.sales_invoice_id=i.id AND c.board_id=i.board_id AND c.status='posted'),0) credited_minor,MAX(0,i.total_minor-i.paid_minor-COALESCE((SELECT SUM(c.total_minor) FROM sales_credit_notes c WHERE c.sales_invoice_id=i.id AND c.board_id=i.board_id AND c.status='posted'),0)) outstanding_minor,(SELECT pl.id FROM payment_links pl WHERE pl.entity_type='sales_invoice' AND pl.entity_id=i.id AND pl.board_id=i.board_id ORDER BY pl.linked_at DESC LIMIT 1) payment_link_id,(SELECT pl.bank_transaction_id FROM payment_links pl WHERE pl.entity_type='sales_invoice' AND pl.entity_id=i.id AND pl.board_id=i.board_id ORDER BY pl.linked_at DESC LIMIT 1) bank_transaction_id,(SELECT pl.status FROM payment_links pl WHERE pl.entity_type='sales_invoice' AND pl.entity_id=i.id AND pl.board_id=i.board_id ORDER BY pl.linked_at DESC LIMIT 1) payment_link_status,COALESCE((SELECT SUM(p.amount_minor) FROM invoice_payments p WHERE p.board_id=i.board_id AND p.entity_type='sales_invoice' AND p.entity_id=i.id AND p.status<>'reversed'),0) recorded_payment_minor,COALESCE((SELECT SUM(p.amount_minor) FROM invoice_payments p WHERE p.board_id=i.board_id AND p.entity_type='sales_invoice' AND p.entity_id=i.id AND p.status='recorded' AND p.bank_transaction_id IS NULL),0) pending_manual_payment_minor,a.company_name,CASE WHEN MAX(0,i.total_minor-i.paid_minor-COALESCE((SELECT SUM(c.total_minor) FROM sales_credit_notes c WHERE c.sales_invoice_id=i.id AND c.board_id=i.board_id AND c.status='posted'),0))=0 THEN 'paid' WHEN i.due_date IS NOT NULL AND i.due_date < date('now') THEN 'overdue' ELSE i.status END AS collection_status FROM sales_invoices i LEFT JOIN crm_accounts a ON a.id=i.account_id WHERE i.board_id=? AND i.status IN ('approved','sent','paid','overdue') ORDER BY CASE WHEN MAX(0,i.total_minor-i.paid_minor-COALESCE((SELECT SUM(c.total_minor) FROM sales_credit_notes c WHERE c.sales_invoice_id=i.id AND c.board_id=i.board_id AND c.status='posted'),0))=0 THEN 3 WHEN i.due_date < date('now') THEN 0 ELSE 1 END,i.due_date",
        )
        .bind(boardId)
        .all()
    ).results;
  ...
  const [accounts, periods, vouchers, intercompany, notes, invoices] =
    await Promise.all([
      ...
      db
        .prepare(
          "SELECT COALESCE(SUM(CASE WHEN outstanding_minor>0 THEN 1 ELSE 0 END),0) AS count,COALESCE(SUM(outstanding_minor),0) AS outstanding_minor,COALESCE(SUM(CASE WHEN due_date IS NOT NULL AND due_date < date('now') AND outstanding_minor>0 THEN outstanding_minor ELSE 0 END),0) AS overdue_minor FROM (SELECT due_date,MAX(0,total_minor-paid_minor-COALESCE((SELECT SUM(c.total_minor) FROM sales_credit_notes c WHERE c.sales_invoice_id=sales_invoices.id AND c.board_id=sales_invoices.board_id AND c.status='posted'),0)) AS outstanding_minor FROM sales_invoices WHERE board_id=? AND status IN ('approved','sent','paid','overdue'))",
        )
        .bind(boardId)
        .first(),
    ]);
