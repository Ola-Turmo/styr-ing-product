...
async function boardData(env: Env, boardId: string, view: string, invoiceId = '') {
  ...
  if (view === 'invoices') return (await db.prepare("SELECT i.*,a.company_name,(SELECT COUNT(*) FROM sales_invoice_lines il WHERE il.sales_invoice_id=i.id AND il.board_id=i.board_id) line_count,COALESCE((SELECT SUM(c.total_minor) FROM sales_credit_notes c WHERE c.sales_invoice_id=i.id AND c.board_id=i.board_id AND c.status='posted'),0) credited_minor,COALESCE((SELECT SUM(c.total_minor) FROM sales_credit_notes c WHERE c.sales_invoice_id=i.id AND c.board_id=i.board_id AND c.status NOT IN ('cancelled')),0) reserved_credit_minor FROM sales_invoices i LEFT JOIN crm_accounts a ON a.id=i.account_id WHERE i.board_id=? ORDER BY CASE i.status WHEN 'overdue' THEN 1 WHEN 'review' THEN 2 WHEN 'approved' THEN 3 ELSE 4 END,i.due_date,i.created_at DESC").bind(boardId).all()).results;
...
export const onRequestPost: PagesFunction<Env> = async ({ env, request }) => {
  try {
    ...
    if (action === 'import_vouchers') {
      const rows = Array.isArray(value?.vouchers) ? value.vouchers as Record<string, unknown>[] : [];
      if (!rows.length || rows.length > 100) return json({ error: 'vouchers_required_max_100' }, { status: 400 });
      const refs = new Set<string>(); const normalizedRows: Array<{ voucherDate: string; period: string; description: string; externalReference: string | null; lines: Array<{ accountId: string; description: string; debit: number; credit: number; vatCode: string | null }> }> = [];
      for (let rowIndex = 0; rowIndex < rows.length; rowIndex += 1) {
        const row = rows[rowIndex]; const voucherDate = String(row.voucherDate || row.voucher_date || '').trim(); const period = String(row.period || voucherDate.slice(0, 7)).trim(); const description = String(row.description || '').trim(); const externalReference = String(row.externalReference || row.external_reference || '').trim() || null; const rawLines = Array.isArray(row.lines) ? row.lines as Record<string, unknown>[] : [];
        if (!datePattern.test(voucherDate) || !periodPattern.test(period) || !description || description.length > 200 || (externalReference && (externalReference.length > 120 || refs.has(externalReference))) || rawLines.length < 2 || rawLines.length > 100) return json({ error: 'voucher_import_row_invalid', row: rowIndex + 1 }, { status: 400 });
        if (externalReference) refs.add(externalReference);
        if (await db.prepare("SELECT 1 FROM accounting_periods WHERE board_id=? AND period=? AND status='locked'").bind(boardId, period).first()) return json({ error: 'period_locked', period, row: rowIndex + 1 }, { status: 409 });
        let debit = 0; let credit = 0; const lines: Array<{ accountId: string; description: string; debit: number; credit: number; vatCode: string | null }> = [];
        for (const line of rawLines) {
          const accountId = String(line.accountId || '').trim(); const lineDescription = String(line.description || description).trim(); const d = asMinor(line.debitMinor ?? line.debit_minor ?? 0); const c = asMinor(line.creditMinor ?? line.credit_minor ?? 0); const vatCode = line.vatCode === undefined || line.vatCode === null || line.vatCode === '' ? null : String(line.vatCode).trim();
          if (!accountId || !lineDescription || lineDescription.length > 200 || d === null || c === null || (d === 0 && c === 0) || (d > 0 && c > 0) || (vatCode !== null && !['0','1','3','3_15','3C'].includes(vatCode))) return json({ error: 'voucher_import_line_invalid', row: rowIndex + 1 }, { status: 400 });
          debit += d; credit += c; lines.push({ accountId, description: lineDescription, debit: d, credit: c, vatCode });
        }
        if (debit !== credit || debit <= 0) return json({ error: 'voucher_import_row_not_balanced', row: rowIndex + 1, debitMinor: debit, creditMinor: credit }, { status: 422 });
        normalizedRows.push({ voucherDate, period, description, externalReference, lines });
      }
      if (refs.size) { const placeholders = [...refs].map(() => '?').join(','); const existingRefs = (await db.prepare(`SELECT external_reference FROM vouchers WHERE board_id=? AND external_reference IN (${placeholders})`).bind(boardId, ...refs).all()).results; if (existingRefs.length) return json({ error: 'voucher_external_reference_exists', references: existingRefs.map((item) => String((item as Record<string, unknown>).external_reference || '')) }, { status: 409 }); }
      const accountIds = [...new Set(normalizedRows.flatMap((row) => row.lines.map((line) => line.accountId)))]; const accountPlaceholders = accountIds.map(() => '?').join(','); const accountRows = (await db.prepare(`SELECT id FROM ledger_accounts WHERE board_id=? AND active=1 AND id IN (${accountPlaceholders})`).bind(boardId, ...accountIds).all()).results; if (accountRows.length !== accountIds.length) return json({ error: 'voucher_import_account_not_found' }, { status: 400 });
      const statements: D1PreparedStatement[] = []; const created: Array<{ voucherId: string; period: string; externalReference: string | null }> = [];
      for (const row of normalizedRows) { const voucherId = id('voucher'); created.push({ voucherId, period: row.period, externalReference: row.externalReference }); statements.push(db.prepare('INSERT INTO voucher_sequences (board_id,next_number) SELECT ?,COALESCE(MAX(voucher_number),0)+1 FROM vouchers WHERE board_id=? ON CONFLICT(board_id) DO NOTHING').bind(boardId, boardId), db.prepare("UPDATE voucher_sequences SET next_number=next_number+1,updated_at=datetime('now') WHERE board_id=?").bind(boardId), db.prepare('INSERT INTO vouchers (id,board_id,voucher_number,voucher_date,period,description,source,status,external_reference,created_by) SELECT ?,?,next_number-1,?,?,?,?,?,?,? FROM voucher_sequences WHERE board_id=?').bind(voucherId, boardId, row.voucherDate, row.period, row.description, 'csv_import', 'posted', row.externalReference, authorization.userId || 'api', boardId)); for (const line of row.lines) statements.push(db.prepare('INSERT INTO voucher_lines (id,voucher_id,account_id,description,debit_minor,credit_minor,vat_code) VALUES (?,?,?,?,?,?,?)').bind(id('line'), voucherId, line.accountId, line.description, line.debit, line.credit, line.vatCode)); }
      await db.batch(statements); for (const item of created) await recordAudit(db, { boardId, action: 'voucher_imported', entityType: 'voucher', entityId: item.voucherId, userId: authorization.userId || undefined, details: { period: item.period, externalReference: item.externalReference, source: 'csv_import' } });
      return json({ ok: true, action, imported: created.length, voucherIds: created.map((item) => item.voucherId), source: 'csv_import' }, { status: 201 });
    }
    ...
    if (action === 'create_credit_note') {
      ...
      if(!invoiceId||!number||!datePattern.test(issueDate)||!description||amountMinor===null||amountMinor<=0||vatMinor===null||vatMinor<0)return json({error:'credit_note_fields_invalid'},{status:400}); const invoice=await db.prepare("SELECT i.total_minor,i.status,COALESCE((SELECT SUM(c.total_minor) FROM sales_credit_notes c WHERE c.sales_invoice_id=i.id AND c.board_id=i.board_id AND c.status NOT IN ('cancelled')),0) reserved_credit_minor FROM sales_invoices i WHERE i.id=? AND i.board_id=? AND i.status NOT IN ('draft','cancelled')").bind(invoiceId,boardId).first<Record<string,unknown>>(); if(!invoice)return json({error:'invoice_not_found_or_not_issued'},{status:409}); const totalMinor=amountMinor+vatMinor; const creditableMinor=Math.max(0,Number(invoice.total_minor||0)-Number(invoice.reserved_credit_minor||0)); if(totalMinor>creditableMinor)return json({error:'credit_note_exceeds_remaining',invoiceTotalMinor:Number(invoice.total_minor||0),creditableMinor},{status:409}); const creditId=id('credit'); const inserted=await db.prepare("INSERT INTO sales_credit_notes (id,board_id,sales_invoice_id,credit_note_number,issue_date,description,amount_minor,vat_minor,total_minor,status,created_by) SELECT ?,?,?,?,?,?,?,?,?,?,? WHERE ? <= (SELECT i.total_minor-COALESCE((SELECT SUM(c.total_minor) FROM sales_credit_notes c WHERE c.sales_invoice_id=i.id AND c.board_id=i.board_id AND c.status NOT IN ('cancelled')),0) FROM sales_invoices i WHERE i.id=? AND i.board_id=? AND i.status NOT IN ('draft','cancelled'))").bind(creditId,boardId,invoiceId,number,issueDate,description,amountMinor,vatMinor,totalMinor,'draft',authorization.userId||'service',totalMinor,invoiceId,boardId).run(); if(!inserted.meta?.changes){const latest=await db.prepare("SELECT MAX(0,i.total_minor-COALESCE((SELECT SUM(c.total_minor) FROM sales_credit_notes c WHERE c.sales_invoice_id=i.id AND c.board_id=i.board_id AND c.status NOT IN ('cancelled')),0)) creditable_minor FROM sales_invoices i WHERE i.id=? AND i.board_id=?").bind(invoiceId,boardId).first<Record<string,unknown>>();return json({error:'credit_note_exceeds_remaining',creditableMinor:Number(latest?.creditable_minor||0)},{status:409});} await recordAudit(db,{boardId,action:'sales_credit_note_created',entityType:'sales_credit_note',entityId:creditId,userId:authorization.userId||undefined,details:{invoiceId,totalMinor,status:'draft',creditableMinorAfter:creditableMinor-totalMinor}}); return json({ok:true,action,creditNoteId:creditId,status:'draft',totalMinor,creditableMinorAfter:creditableMinor-totalMinor},{status:201});
    }
    ...
    if (action === 'record_invoice_payment') {
      ...
      const invoice=await db.prepare("SELECT i.total_minor,i.paid_minor,i.status,COALESCE((SELECT SUM(c.total_minor) FROM sales_credit_notes c WHERE c.sales_invoice_id=i.id AND c.board_id=i.board_id AND c.status='posted'),0) credited_minor FROM sales_invoices i WHERE i.id=? AND i.board_id=?").bind(invoiceId,boardId).first<Record<string,unknown>>(); if(!invoice)return json({error:'invoice_not_payable_or_found'},{status:409});
      const total=Number(invoice.total_minor||0), alreadyPaid=Number(invoice.paid_minor||0), credited=Number(invoice.credited_minor||0), remaining=Math.max(0,total-alreadyPaid-credited);
      const existingPayment=await db.prepare("SELECT id,amount_minor,status FROM invoice_payments WHERE board_id=? AND entity_type='sales_invoice' AND entity_id=? AND bank_transaction_id IS NULL AND payment_reference=? AND status<>'reversed' LIMIT 1").bind(boardId,invoiceId,paymentReference).first<Record<string,unknown>>(); if(existingPayment){const existingAmount=Number(existingPayment.amount_minor||0); if(requestedAmount!==null && existingAmount!==requestedAmount)return json({error:'payment_reference_conflict',detail:'Betalingsreferansen er allerede brukt med et annet beløp.',existingPaymentId:String(existingPayment.id),existingAmountMinor:existingAmount},{status:409}); return json({ok:true,action,invoiceId,status:String(invoice.status),paymentSource:'manual',paymentId:String(existingPayment.id),amountMinor:existingAmount,paidMinor:alreadyPaid,remainingMinor:remaining,idempotent:true});}
      if(!['approved','sent','overdue'].includes(String(invoice.status))) return json({error:'invoice_not_payable_or_found'},{status:409}); const paymentAmount=requestedAmount ?? remaining; if(paymentAmount<=0 || paymentAmount>remaining) return json({error:'payment_exceeds_remaining',remainingMinor:remaining,creditedMinor:credited},{status:409});
      const nextPaid=alreadyPaid+paymentAmount, nextStatus=nextPaid+credited>=total?'paid':invoice.status; const paymentId=id('ipmt');
