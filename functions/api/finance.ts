  ...
  try {
    ...
    if (action === 'create_invoice') {
      ...
      for (let index = 0; index < rawLines.length; index += 1) {
        const line = rawLines[index]; const lineDescription=String(line.description||'').trim(); const quantity=Number(line.quantity ?? 1); const unitPriceMinor=Number(line.unitPriceMinor ?? line.unit_price_minor ?? 0); const vatRate=Number(line.vatRate ?? line.vat_rate ?? 0); const vatCode=line.vatCode ? String(line.vatCode).trim() : (vatRate ? '3' : null); const lineAccountId=String(line.accountId ?? line.account_id ?? '').trim() || null;
