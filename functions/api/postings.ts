...
export const onRequestGet: PagesFunction<Env> = async ({ env, request }) => {
  ...
  try {
    ...
    if (view === 'proposals') {
      ...
      const data = [...(invoiceRows.results || []), ...(payrollRows.results || [])].sort((a, b) => String(b.created_at || '').localeCompare(String(a.created_at || '')));
      return json({ boardId, view, data });
    }
    const [review, approved, posted] = await Promise.all([
      db.prepare("SELECT (SELECT COUNT(*) FROM posting_proposals WHERE board_id=? AND status='review') + (SELECT COUNT(*) FROM payroll_posting_proposals WHERE board_id=? AND status='review') AS count").bind(boardId, boardId).first(),
      db.prepare("SELECT (SELECT COUNT(*) FROM posting_proposals WHERE board_id=? AND status='approved') + (SELECT COUNT(*) FROM payroll_posting_proposals WHERE board_id=? AND status='approved') AS count").bind(boardId, boardId).first(),
      db.prepare("SELECT (SELECT COUNT(*) FROM posting_proposals WHERE board_id=? AND status='posted') + (SELECT COUNT(*) FROM payroll_posting_proposals WHERE board_id=? AND status='posted') AS count").bind(boardId, boardId).first(),
    ]);
...
export const onRequestPost: PagesFunction<Env> = async ({ env, request }) => {
  try {
    ...
    return json({ error: 'unknown_action', allowed: ['prepare_sales_invoice', 'prepare_supplier_invoice', 'prepare_payroll', 'approve_proposal', 'approve_payroll_proposal', 'post_proposal', 'post_payroll_proposal'] }, { status: 400 });
  } catch (error) { return json({ error: 'database_unavailable', detail: error instanceof Error ? error.message : 'unknown' }, { status: 503 }); }
};
