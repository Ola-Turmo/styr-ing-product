...
export const onRequestPost: PagesFunction<Env> = async ({ env, request }) => {
  try {
    ...
    if (action === 'create_run') {
      ...
      if (!periodPattern.test(period) || gross === null || tax === null || employer === null || !Number.isInteger(employees) || employees < 0 || employees > 100000) return json({ error: 'payroll_run_fields_invalid' }, { status: 400 });
    ...
    if (action === 'calculate_compliance') {
      const runId = txt(value?.payrollRunId, 100); const run = await db.prepare('SELECT gross_minor, employee_count FROM payroll_runs WHERE id=? AND board_id=?').bind(runId, boardId).first<Record<string, unknown>>(); if (!run) return json({ error: 'payroll_run_not_found' }, { status: 404 });
      ...
      const gross = Number(run.gross_minor || 0); const holiday = Math.round(gross * holidayRate); const otp = Math.round(gross * otpRate); const checkId = id('paycheck'); const assumptions = JSON.stringify({ holidayBasis: 'gross wages', otpBasis: 'pensionable salary', externalRules: 'requires payroll review' });
    ...
    if (action === 'approve_compliance') {
      const checkId = txt(value?.checkId, 100); const result = await db.prepare("UPDATE payroll_compliance_checks SET status='approved', reviewed_by=?, reviewed_at=datetime('now') WHERE id=? AND board_id=? AND status IN ('calculated','review')").bind(actor, checkId, boardId).run(); if (!result.meta?.changes) return json({ error: 'compliance_check_not_open_or_found' }, { status: 409 }); await recordAudit(db, { boardId, action: 'payroll_compliance_approved', entityType: 'payroll_compliance_check', entityId: checkId, userId: authorization.userId || undefined, details: { externalSubmission: 'not_configured' } }); return json({ ok: true, action, checkId, status: 'approved', externalSubmission: 'not_configured' });
    }
    ...
    if (action === 'prepare_submission') {
      const submissionType = ['a_melding', 'tax_return', 'mva', 'nav_income', 'annual_accounts'].includes(txt(value?.submissionType, 30)) ? txt(value?.submissionType, 30) : ''; const period = txt(value?.period, 20); if (!submissionType || !period) return json({ error: 'submission_fields_invalid' }, { status: 400 });
      const submissionId = id('submission'); const payloadHash = await crypto.subtle.digest('SHA-256', new TextEncoder().encode(JSON.stringify({ boardId, submissionType, period, preparedBy: actor }))).then((b) => Array.from(new Uint8Array(b)).map((x) => x.toString(16).padStart(2, '0')).join(''));
