...
const specs: SearchSpec[] = [
  ...
  { table: 'vouchers', type: 'Bokført bilag', title: "'Bilag ' || voucher_number || ' · ' || description", snippet: "coalesce(voucher_date,'') || ' · ' || coalesce(period,'') || ' · ' || coalesce(source,'') || ' · ' || coalesce(status,'')", source: 'Hovedbok' },
  { table: 'sales_invoices', type: 'Kundefaktura', title: "invoice_number || ' · ' || description", snippet: "coalesce(issue_date,'') || ' · Forfall: ' || coalesce(due_date,'') || ' · ' || coalesce(status,'') || ' · ' || coalesce(total_minor,'') || ' øre'", source: 'Kundefakturaer' },
  { table: 'ledger_accounts', type: 'Regnskapskonto', title: "code || ' · ' || name", snippet: "coalesce(account_type,'') || ' · MVA: ' || coalesce(vat_code,'') || ' · ' || coalesce(active,'')", source: 'Kontoplan' },
