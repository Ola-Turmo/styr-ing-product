...
async function calculate(db: D1Database, boardId: string, period: string) {
  ...
  const snapshot = JSON.stringify({ basis: 'voucher_lines', rateAssumptions: { '1': 0.25, '1_15': 0.15, '3': 0.25, '3_15': 0.15, '3C': -0.25 }, lines });
...
export const onRequestPost: PagesFunction<Env> = async ({ env, request }) => {
  try {
    ...
    if (action === 'prepare_submission') {
      if (row.status !== 'approved') return json({ error: 'vat_period_not_approved' }, { status: 409 }); const payloadHash = await sha256(JSON.stringify({ boardId, period, basisMinor: row.basis_minor, outputVatMinor: row.output_vat_minor, inputVatMinor: row.input_vat_minor, sourceHash: row.source_hash })); const submissionId = id('submission');
      await db.prepare("INSERT INTO compliance_submissions (id,board_id,submission_type,period,status,payload_hash,notes,created_at) VALUES (?,?,?,?,?,?,?,datetime('now'))").bind(submissionId, boardId, 'mva', period, 'prepared', payloadHash, 'MVA-grunnlag klargjort internt; ekstern innsending ikke konfigurert').run(); await db.prepare("UPDATE vat_periods SET status='prepared',submission_id=?,updated_at=datetime('now') WHERE id=? AND board_id=?").bind(submissionId, row.id, boardId).run(); await recordAudit(db, { boardId, action: 'vat_submission_prepared', entityType: 'vat_period', entityId: String(row.id), userId: authorization.userId || undefined, details: { period, submissionId, externalSubmission: 'not_configured' } }); return json({ ok: true, action, period, submissionId, status: 'prepared', payloadHash, externalSubmission: 'not_configured' }, { status: 201 });
    }
