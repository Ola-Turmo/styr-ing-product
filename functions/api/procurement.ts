...
export const onRequestPost: PagesFunction<Env> = async ({ env, request }) => {
  try { const v = await body(request); const boardId = String(v?.boardId || '').trim(); const action = String(v?.action || '').trim(); const db = requireDb(env); if (!boardId || !action) return json({ error: 'boardId_and_action_required' }, { status: 400 }); const authorization = await authorizeBoardWrite(request, env, boardId); if (!authorization.allowed) return json({ error: 'write_not_authorized' }, { status: 401 });
    if (action === 'approve_order') { const orderId = String(v?.orderId || ''); const result = await db.prepare("UPDATE purchase_orders SET status='approved',approved_by=?,approved_at=datetime('now') WHERE id=? AND board_id=? AND status='pending_approval'").bind(v?.approvedBy || authorization.userId || 'api', orderId, boardId).run(); if (!result.meta?.changes) return json({ error: 'order_not_pending_or_found' }, { status: 409 }); await recordAudit(db, { boardId, action: 'purchase_order_approved', entityType: 'purchase_order', entityId: orderId, userId: authorization.userId || undefined, details: { externalOrdering: 'not_configured' } }); return json({ ok: true, action, orderId, status: 'approved', externalOrdering: 'not_configured' }); }
    if (action === 'match_invoice') { const invoiceId = String(v?.invoiceId || ''); const result = await db.prepare("UPDATE supplier_invoices SET match_status='matched',status='matched' WHERE id=? AND board_id=? AND status='received'").bind(invoiceId, boardId).run(); if (!result.meta?.changes) return json({ error: 'invoice_not_received_or_found' }, { status: 409 }); await recordAudit(db, { boardId, action: 'supplier_invoice_matched', entityType: 'supplier_invoice', entityId: invoiceId, userId: authorization.userId || undefined, details: { requiresHumanApproval: true } }); return json({ ok: true, action, invoiceId, status: 'matched', requiresHumanApproval: true }); }
